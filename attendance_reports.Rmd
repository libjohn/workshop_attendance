...
attendance_headers_names <- names(attendance_headers)
# attendance_headers_names
# glue::glue_collapse(rep("=", 20))
# names(my_df)
```
...
my_df <- my_df %>% 
  # fill(c(`Workshop Date`, `Workshop Name` , WorkshopID), .direction = "down") %>% 
  fill(c(`Workshop Name`), .direction = "down") %>% 
  drop_na(Attendance) %>%
