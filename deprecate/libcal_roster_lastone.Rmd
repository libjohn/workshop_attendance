...
#  rename(relpath = value) %>% 
  ...
  mutate(reg_data = 
           map(relpath, read_csv, skip = 9)) %>%  # jrl was 8
...
roster2 %>% 
  ...
  labs(title = "Attendance by Discipline",
       ...
       x = "", y = "")
ggsave("outfile/who_we_are2.svg", device = "svg")
# ggsave("C:/Users/jrl/Documents/workshop_rfun_flipped/slides/images/who_we_are2.svg", device = "svg")
```


```{r echo=FALSE, fig.height=8, message=FALSE, warning=FALSE, dev='svg'}
roster2 %>% 
  mutate(discipline = fct_lump_min(discipline, min = 2)) %>% 
  ggplot() +
  geom_bar(aes(x = fct_rev(discipline), 
...
ggsave("outfile/who_we_are_facet.svg", device = "svg")
# ggsave("C:/Users/jrl/Documents/workshop_rfun_flipped/slides/images/who_we_are_facet.svg", device = "svg")
```
