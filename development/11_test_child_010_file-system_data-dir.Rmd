...
roster <- full_file_names %>% 
  select(`Last Name`, `First Name`, `Email`, 
         ...
         relpath) %>% 
  mutate(Registration = str_to_title(str_extract(relpath, "(attendees|wait_list)"))) %>% 
  select(-relpath) %>% 
  arrange(`Last Name`) 
```
...
for_goog <- full_file_names %>% 
  ...
  #select("Workshop Name", "Workshop Date") 
for_goog  
...
attendees_header %>% spread(X1, X2) %>% select(Event) %>% toString()

str_extract("\\w+\\s\\d{1,2},\\s\\d{4}") %>% lubridate::mdy()

attendees_header %>% spread(X1, X2) %>% select(Date) %>% mdy()
```
