...
date: "`r Sys.Date()`"
params:
  upload: FALSE  # if you want to upload to google, change this option to TRUE
---
...
for_goog <- full_file_names %>% 
  ...
  mutate("Workshop Date" = attendees_header %>% spread(X1, X2) %>% select(Date) %>% mdy()) %>% 
  ...
  separate(metadata_filename, into = c("prefix", "type", "workshop_id")) %>% 
  separate(workshop_id, into = c("WorkshopID", "pid"), sep = c(7)) %>% 
...
Upload to Google as a sheet

```{r upload_to_googledrive, eval=params$upload, include=params$upload}
sheetname <- paste(for_goog$`Workshop Name`[1], for_goog$`Workshop Date`[1], sep = "_")
local_filename <- str_remove_all(sheetname, "[:\\.\\s,]")
