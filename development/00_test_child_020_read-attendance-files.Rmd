...
```{r}
Atteendees1 <- read_csv(full_file_names$relpath[1], skip = 9)

full_file_names %>% 
  #mutate(relpath2 = as.character(relpath)) %>% 
  map_dfr(read_csv, select(relpath))

length(full_file_names$relpath)

#foo <- fs::dir_ls(".")
#rm(foo)

#as.character(full_file_names$relpath)


# https://www.gerkelab.com/blog/2018/09/import-directory-csv-purrr-readr/
foo <- map_dfr(full_file_names$relpath, read_csv, skip = 9, .id = "source")
